...
`src/run_sorting_demo.py` 是建置展示用的少量投料場景。固定 seed 會產生 `MIDDLE` 與 `RIGHT` 工件，中央協調器先安排共享區與專屬區任務，再讓兩台 Nova5 以位置 IK 執行接近、下降、夾爪閉合、抬升、移至實體托盤、放開與回原位。工件不會附著到工具座標；只有 MuJoCo 回報指墊與工件的實體接觸才會承認抓取，且放置必須由工件最後落入目標托盤的範圍驗證。
