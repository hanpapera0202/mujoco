...
`run_sorting_demo.py` 已把高階分派接到 Nova5 的位置 IK。每個任務依序經過接近、下降、夾爪閉合、抬升、移動到左右實體托盤、下降、張開、撤離與回原位。夾爪閉合後使用運動學附著維持物件相對於工具的位置，目標是讓中央協作流程在固定 seed 下可穩定重現；它不是接觸摩擦抓取的最終物理驗證。
