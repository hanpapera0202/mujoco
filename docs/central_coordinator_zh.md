...
`run_sorting_demo.py` 已把高階分派接到 Nova5 的位置 IK。每個任務依序經過接近、下降、夾爪閉合、抬升、移動到左右實體托盤、下降、張開、撤離與回原位。夾爪閉合後只在 MuJoCo 回報指墊接觸工件時啟用 MuJoCo 夾持約束；接觸前不會移動或附著工件，張開時立即解除約束。完成時還會驗證工件是否真正在目標托盤內。
