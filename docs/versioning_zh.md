...
目前版本 `0.8.0` 延續 BC-JSP，將動態抓取改為雙速率閉迴路：MuJoCo 狀態 500 Hz、熱啟動 IK 25 Hz，準備、跟帶、下降與閉爪期間持續更新目標。投料橫向位置使用固定 seed 可重現的隨機數，GUI 可調隨機範圍；預設批次左右分散。`PLACE-01` 的 10 件 100% 與 10 cm 預設下更高雙臂重疊運動比例仍列為尚未完成。
