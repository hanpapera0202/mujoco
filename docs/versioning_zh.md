...
目前版本 `0.12.0` 延續 BC-JSP、中央交接預備與雙速率閉迴路 IK，並加入執行器效能節流與並行 handoff 接近。安全待命位仍以完整未來路徑驗證；開始接近後改以 1 秒短視窗持續檢查，讓兩臂保持平行權限，不再把共同走廊淨空當成右臂開始移動的必要條件。追蹤 IK 採 12.5 Hz、12 次 warm-start 迭代與 8 mm 位移門檻；安全預測採 50 Hz，實體 MuJoCo 接觸仍維持每個控制步檢查。
