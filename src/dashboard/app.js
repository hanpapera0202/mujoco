...
const zh = {
  ...
  infeed: '投料', assign: '派工', grasp: '抓取', place: '放置', missed: '漏件',
...
function render(state) {
  ...
  if (!initialized) { fields.forEach(field => field.value = field.name === 'seed' ? state.seed : state.parameters[field.name]); initialized = true; }
...
document.querySelector('#settings').onsubmit = event => { event.preventDefault(); const values = Object.fromEntries(fields.map(field => [field.name, field.name === 'seed' ? Number.parseInt(field.value, 10) : Number.parseFloat(field.value)])); control('settings', values).then(render).catch(error => window.alert(error.message)); };
